...
(function (w) {
  ...
  function refreshGameLayer(box, loop, offset) {
    ...
    for (let j = 0; j < box.children.length; j++) {
      ...
      if (i === j) {
        ...
        r.notEmpty = true;
        i =
          (Math.floor(j / 4) + 1) * 4 + (Math.floor(Math.random() * 1000) % 4);
      } else {
        r.notEmpty = false;
      }
    }
  ...
  function getBestScore(score) {
    ...
    let best = cookie(cookieName)
      ? Math.max(parseFloat(cookie(cookieName)), score)
      : score;
  ...
  function showGameScoreLayer(cps) {
    ...
    $("#GameScoreLayer-score")
      ...
      .html(
        "得分&nbsp;" +
          (normalCond ? score : "<span style='color:red;'>" + score + "</span>")
      );
